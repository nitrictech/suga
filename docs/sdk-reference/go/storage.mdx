...
```go
content, err := app.Files.Read("path/to/file.txt")
if err != nil {
    log.Fatal(err)
}
...
```go
err := app.Files.Write("path/to/file.txt", []byte("Hello, World!"))
if err != nil {
    log.Fatal(err)
}
```
...
```go
err := app.Files.Delete("path/to/file.txt")
if err != nil {
    log.Fatal(err)
}
```
...
```go
// List all files
files, err := app.Files.List("")
if err != nil {
    log.Fatal(err)
}

// List files in a specific directory
docs, err := app.Files.List("documents/")
if err != nil {
    log.Fatal(err)
}
...
```go
exists, err := app.Files.Exists("config.json")
if err != nil {
    log.Fatal(err)
}
...
downloadURL, err := app.Files.GetDownloadURL("document.pdf")
